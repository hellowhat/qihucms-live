...
@section('content')
    <div class="live-main" id="LiveMain" v-cloak>
        <div id="pusherView" class="pushView"></div>
        ...
        <div class="live-content">
            ...
            <div class="live-bottom">
                ...
                <div class="live-bottom-button" v-if="!texting">
                    <button v-on:click="QhNavigate('{{route('live.wap.index')}}',true,false)">
...
@endsection
@push('scripts')
    <script src="{{asset('asset/live/tweblive.js')}}"></script>
    <script src="{{asset('asset/live/popper.js')}}"></script>
    <script src="{{asset('asset/live/bootstrap-material-design.js')}}"></script>
    <script>
        $(document).ready(function () {
            $('body').bootstrapMaterialDesign();
        });
    </script>
    <script src="{{asset('asset/live/lib-generate-test-usersig.min.js')}}"></script>
    <script src="{{asset('asset/live/trtc.js')}}"></script>
    <script src="{{asset('asset/live/common.js')}}"></script>
    <script src="{{asset('asset/live/rtc-client.js')}}"></script>
    <script src="{{asset('asset/live/share-client.js')}}"></script>
    <script src="{{asset('asset/live/presetting.js')}}"></script>
    <script src="{{asset('asset/live/device-testing.js')}}"></script>
    <script src="{{asset('asset/live/index.js')}}"></script>
    <script src="https://cdn.bootcdn.net/ajax/libs/vue/2.6.9/vue.min.js"></script>
    <script>
        // ------------- 当需要推流时，创建 Pusher（推流）对象，最简单的推流仅需3步 -------------
        // 1、创建 Pusher（推流）对象
        let pusher = TWebLive.createPusher({ userID: '{{$room->user->id}}' });
        let SDKAppID = '{{cache('config_live_tencent_sdkappid')}}';
        let live = '{{$room->user->id}}';
        let userID = '{{$room->user->id}}';
        let userSig = '{{$sig}}';
        let liveDomainName = '{{cache('config_live_tencent_pullurl')}}';
        let streamID = 'room-{{$room->user->id}}';
        ...
        pusher.setRenderView({
            ...
            // url 必须以 `room://` 开头
            let url = `room://sdkappid=${SDKAppID}&roomid=${live}&userid=${userID}&usersig=${userSig}&livedomainname=${liveDomainName}&streamid=${streamID}`;
        ...
        const Live = new Vue({
            ...
            methods: {
                ...
                appReset: function(platform,statusBarHeight){
                    ...
                    if(Live.screen){
                        $("#LivePlayer").css("padding-top",statusBarHeight + 'px');
                    }else{
                        $("#LivePlayer").height(Live.height + statusBarHeight);
                    };
                    switch(platform){
                        case 'android':
